...
public class C19EqualsNHashcode {
	...
	public static void main(String[] args) {
		...
		//컬렌션 프레임워크의 동일성(identity)과 동등성(equlity) 이 모두 '참'이면 같은 값으로 판단.
		Set<Customer> customers = new HashSet<>();
		
		customers.add(new Customer("momo","모모",23));
		customers.add(new Customer("jenny","제니",22));
		customers.add(new Customer("nayeon","나연",21));
		...
		// 		-> 3개의 필드값으로 해시코드 계산하고
		//		-> 3개의 필드값이 모두 같으면 equals '참'이면 리턴
		Set<CustomerENH> customers2 = new HashSet<>();
		
		customers2.add(new CustomerENH("momo","모모",23));
		customers2.add(new CustomerENH("jenny","제니",22));
		customers2.add(new CustomerENH("nayeon","나연",21));
		...
		System.out.println("\n참고용");
		System.out.println(cs1==cs2);  //toString이 숨겨져있음. //false
		System.out.println(cs1.toString()==cs2.toString());  //false
		//toString 으로 새로 만들어진 문자열 객체의 비교 입니다.
	}
}
