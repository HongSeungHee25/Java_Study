...
public class CustomerENH {
	...
	public boolean equals(Object obj) { 	//this와 인자로 받은 obj 를 비교
		...
		if(id == null) {					//this의 id가 null이고 비교대상 id가 null 이 아닐 때
			...
			return false;
		if(name == null) {				//this의 name이 null이고 비교대상 name가 null이 아닐 때
			if(other.name != null)
				return false;
		}else if(!name.equals(other.name))	//this와 비교대상 name이 다를 때
			return false;
		return true;
		//위의 6개 이외의 나머지 경우 즉, id와 name,age 필드값이 모두 같을 때(null 제외)는 '참'
	}
	...
	public CustomerENH(String id, String name, int age) {
		super();
		this.id = id;
		this.name = name;
		this.age = age;
