...
public class C09StringSplitTest {
	...
	public static void main(String[] args) {
		...
		result = temp.split("xy");		//2)에서 정확이 문자열 "xy" 기준으로만 분리. 배열 크기 = 5
		System.out.println("temp 문자열 분리 결과 배열 : "+Arrays.toString(result));
		System.out.println("result.length = "+result.length);
		for(String t : result)
			System.out.println(t);
		
		//split 메소드는 분리된 결과를 배열로 나타냄. 토그나이저는 nextToken()으로 직접 하나씩 가져오기
		System.out.println("\n2. 문자열 토크나이저(토큰 : ");
		temp = "모모 90 88 79";
...
//		System.out.println(stk.nextToken());		//오류 : next로 가져올 데이터 없음.
		...
		System.out.println("\n2. 반복문으로 실행하기 : hasMoreTokens - 더 가져올것이 있는가?");
		int count = 0;
		while (stk.hasMoreTokens()) {
			System.out.println("count = "+count++ +", 다음 토큰 값 = "+stk.nextToken());
			//count++ 는 println 후에 증가(++)
		}
		System.out.println("총 토큰 값 갯수 = "+count);		//예상 값 = 4
		...
		count = 0;
		while (stk.hasMoreTokens()) {	//많이 사용하니까 친해져야함.
			System.out.println("count = "+count++ +", 다음 토큰 값 = "+stk.nextToken());
			//count++ 는 println 후에 증가(++)
		}
		System.out.println("총 토큰 값 갯수 = "+count);		//예상 값은 1)번은 => 5, 2)번은 => 7
