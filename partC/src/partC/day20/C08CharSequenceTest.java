...
public class C08CharSequenceTest {
	//CharSequence(순서대로 나열된 문자) 인터페이스 : String, StringBuffer, StringBuilder 가 구현 클래스(오버라이드)
	public static void main(String[] args) { // CharSequence >> StringBuffer, StringBuilder >> String
	//복습 : 해시코드는 객체의 주소로 만들어지는 객체의 식별을 위한 값입니다.
	//		Integer.toHexString 는 정수(인자)를 16진수로 변환하여 문자열 리턴.16진수 구별을 "0x" 을 앞에 붙입니다.
		...
		//해시코드값이 같다면 동일한 객체에 값의 상태가 바뀌는 것이고 그렇지 않다면 객체가 새로 만들어지는 것.
		...
		for(int i = 0; i < 5; i++) {
			...
			System.out.println(String.format("중간결과 sb 객체 해시코드 ↓ \n 10진수 = %d , 16진수 = 0x%8s\n", sb.hashCode(),
					Integer.toHexString(sb.hashCode())));
		}
		System.out.println("===StringBuffer(StringBuilder) 객체는 객체의 문자열을 변결할 수 있습니다.(가변 객체)\n"
		...
		System.out.println("\n3. String 클래스의 contains 메소드 인자는 CharSequence 타입 입니다.");
		String sample = "abc0/1/2/3/4/xyz";
		CharSequence cs = result; 		//CharSequence 인터페이스 타입으로 업캐스팅.
		System.out.println("sample.contains(cs) = "+sample.contains(cs));
		cs = sb;
		System.out.println("sample.contains(sb) = "+sample.contains(sb));
