...
public class A34ArrayBasic {
	...
	public static void main(String[] args) {
		...
		//배열 선언은 저장할 데이터의 형식,이름과 데이터의 개수 (할당 받는 메모리 크기)
		int[] arr1 = {34,56,75,89,65}; 		//저장할 값 초기화와 크기 결정(4바이트 x 5 = 20바이트 할당)
		//인덱스는 0부터 4
		
		int[] arr2 = new int[10]; 			//초기값은 기본값0, 정수 10개(40바이트 할당), 인덱스 0~9
		
		//배열의 주요 속성 : length 는 배열의 길이(개수)를 갖습니다.
		System.out.println("==인덱스 직접 지정해서 배열 요소값 출력하기==");
		System.out.println("1=0, arr1[0] = "+arr1[0]);
		System.out.println("1=1, arr1[1] = "+arr1[1]);
		System.out.println("1=2, arr1[2] = "+arr1[2]);
		System.out.println("1=3, arr1[3] = "+arr1[3]);
		System.out.println("1=4, arr1[4] = "+arr1[4]);
		...
		System.out.println(String.format("arr1[%d] = %d\n", i,arr1[i]));
		sum += arr1[i];
...
//		int rnd = (int)d;
		...
		//Arrays 클래스는 배열과 관련 static 메소드 있습니다.
		System.out.println("==Arrays 클래스 메소드 연습==");
		System.out.println("배열요소의 값을 for문 없이 출력");
		System.out.println("\nArrays.toString(arr1) = "+Arrays.toString(arr1)); //인자는 int[] 배열, 리턴타입이 String
		System.out.println("Arrays.toString(arr2) = "+Arrays.toString(arr2));
		
		System.out.println("\n배열요소의 값을 순서대로 나열(정렬:sort)");
		Arrays.sort(arr1); //정렬한 결과 arr1 배열은 인덱스의 값들 변경
		System.out.println("Arrays.toString(arr1) = "+Arrays.toString(arr1)); //인자는 int[] 배열, 리턴타입이 String
		Arrays.sort(arr2);
		System.out.println("Arrays.toString(arr2) = "+Arrays.toString(arr2));
	}
