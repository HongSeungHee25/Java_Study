...
public class B14ShapeV2Test {
	...
	public static void main(String[] args) {
		...
		//1) 클래스 캐스팅1 : 부모타입으로 자식 객체를 참조할 수 있습니다.(자동 캐스팅) - upCasting
		Shape sh1 = new LadderShape("사나 사다리꼴-2", 17, 13, 21);
		...
		//2) 클래스 캐스팅2 : 자식타입 변수로 부모타입 참조 객체를 변환할 수 있나요? 불가능(강제 캐스팅) - downCasting
...
//		LadderShape shape = (LadderShape) new Shape("도형",0,0); //문법적으로는 ok, 그러나 실행시 Exception 발생
//		shape.iamLadder();		//오류 : 실제 객체가 Shape 이므로 오류
		
		//문제. 아래 명령어는 실행 가능?
		System.out.println("Circle 객체를 Shape 타입으로 참조하고 그것을 다시 LadderShape으로 캐스팅 가능?");
//		LadderShape ls3 = (LadderShape) sh5;	//문법적으로는 ok
//		ls3.iamLadder();	//오류 : 실제 객체가 Circle 이므로 오류
		
		//자식 클래스의 메소드 iamLadder()는 실제 객체를 LadderShape 일때만 오류없이 실행.
		//3) 강제 캐스팅에는 타입 체크가 필요합니다. - intence of
