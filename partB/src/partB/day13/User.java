...
//불변객체로 정의하는 예시.
//1. 필드는 모두 final   2. 커스텀 생성자 모든 필드 초기화   3. getter 메소드만 정의
public class User {
	//상수 적용 예시
	...
	//모든 필드값 출력 문자열
	public String user() {
		return String.format("id = %s, name = %s, age = %d, point = %.2f ", id,name,age,point);
	}
	//변경이 필요한 필드 point 는 setter 메소드
	public void setPoint(double point) {
		//if(point<=MAX_POINT)  this.point = point; else this.point = MAX_POINT;
		this.point = Math.min(point, MAX_POINT);	//if 안쓴 방법의 코드
	}
